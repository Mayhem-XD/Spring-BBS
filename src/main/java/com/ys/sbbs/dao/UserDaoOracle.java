...
public interface UserDaoOracle {
	...
	@Select("select * from ( select rownum as rnum,"
			+ " u.* from users u where isdeleted=0 and rownum <= #{maxrow})"
			+ " where rnum > #{offset} order by regDate desc, \"uid\"")
	...
	@Update("update users set pwd=#{pwd}, uname=#{uname}, email=#{email}, \"profile\"=#{profile}, addr=#{addr} where \"uid\"=#{uid}")
